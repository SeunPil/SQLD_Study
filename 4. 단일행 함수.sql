...
-- 상수문자열과의 비교
-- 비교 대상이 Varchar2 타입이라 문자열도 Varchar2로 받아짐.
SELECT 
    REPLACE(char_4, ' ', '+') AS char_4,
    REPLACE(varchar_6, ' ', '+') AS varchar_6
FROM varchar_compare
...
-- Java Programmer을 찾아 Java를 BigData로 변경한다.
    REPLACE('Java Programmer', 'Java', 'BigData') AS "REPLACE" -- 문자를 변경

-- 자바를 제거
--    REPLACE('Java Programmer', 'Java') AS "REPLACE" -- 문자를 변경
FROM dual;
...
SELECT 
    ...
    ROUND(38.678, 2) AS round, -- 자리수까지 반올림 ( 소수점 뒤로 기준 )
    TRUNC(38.678, 2) AS trunc,  -- 자리수 이하를 절삭 ( // )
    ABS(-20) AS abs, -- 절대값
    SIGN(50) AS sign -- 0보다 작으면 -1, 0보다 크면1, 0이면 0 (음수 양수 판단)
FROM dual;
...
SELECT 
    sal_cd, sal_nm,
--        sal_cd가 ~할 때 = 100001번일 때 THEN'기본급여'
